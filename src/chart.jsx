    ...
    return (
      <div className="chart-tip" style={{ position: "static" }}>
        <div className="chart-tip-date">
          ...
          {" · "}
          {new Date(label).toLocaleTimeString(lang === "de" ? "de-DE" : "en-GB",
      ...
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={sorted} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
          ...
          <Line dataKey="sys" stroke="var(--accent)" strokeWidth={1.75}
                dot={{ r: 2.5, fill: "var(--bg)", stroke: "var(--accent)", strokeWidth: 1.5 }}
                activeDot={{ r: 4 }} connectNulls isAnimationActive={false} />
          <Line dataKey="dia" stroke="var(--accent)" strokeWidth={1.5} strokeOpacity={0.45}
                dot={{ r: 2.5, fill: "var(--bg)", stroke: "var(--accent)", strokeOpacity: 0.55, strokeWidth: 1.5 }}
                activeDot={{ r: 4 }} connectNulls isAnimationActive={false} />
